...
public class dirtraverse {
	...
	public static void main(String[] args) throws IOException{
		...
		for(int i = 1; i <= N; i++) {
			if(!fil.contains(nodes[i])) {
			HashSet<Node> visited = new HashSet<Node>();
			run(nodes[i],nodes[i], visited,len,0,fil);
			}
		}
	...
	private static void run(Node o,Node f, HashSet<Node> v, long[] len, long cur,HashSet<Node> fil) {
		v.add(f);
		for(Edge e : f.edges) {
			if(!v.contains(e.end)&&!fil.contains(e.end)) {
				...
				if(e.up)
					...
					val=e.end.name.length()+1;
				run(o,e.end,v,len,cur+val,fil);
			}
			else if(fil.contains(e.end)) {
				len[o.id]+=cur+e.end.name.length()+1;
			}
		}
	}
}
